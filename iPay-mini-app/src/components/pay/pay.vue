  ...
  <div class="wrap" v-if="payType.length > 0 && payNum.length >0">
    <div class="phone">
        <div class="phone-wrap">
            ...
            <input type="number" class="phone-input" :maxlength="11" ref="input"/>
            <img src="../../assets/icon_contacts.png" class="phone-contacts" />
        </div>
        <div class="module-wrapper">
            <!--服务商模块-->
            <div class="brands-wrapper">
                <div class="brand-img-wrapper" v-for="(item,index) in payType" @click="selectBrands(item,index)" :class="{'active': brandIndex === index}">
                    <img :src="item.imgUrl" class="brand-img">
                </div>
            </div>
            <!--价格模块-->
            <div class="price-wrap">
                <div class="price-item" v-for="(item,index) in payNum" @click="selectPrice(item,index)" :class="{'active': priceIndex === index}">
  ...
  methods: {
    ...
    pay() {
        let value =  {
            ...
            brands: this.brands,
            price:  this.price,
            elsePrice: this.$refs.elseInput.value.trim()
        }
        this.$emit("pay", value);
    }
  }
